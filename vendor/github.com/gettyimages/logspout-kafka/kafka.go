...
var KafkaProducerMap = make(map[string]sarama.AsyncProducer)
...
func NewKafkaAdapter(route *router.Route) (router.LogAdapter, error) {
	...
	if producer, ok := KafkaProducerMap[strings.Join(brokers, ",")]; ok {
		log.Println("producer already have, return directly")
		return &KafkaAdapter{
			route:    route,
			brokers:  brokers,
			topic:    topic,
